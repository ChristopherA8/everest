...
{
    CGRect frame = self.frame;
    __block UIDynamicAnimator *animator = [self dc_animator];
    if(!animator.isRunning) {
        [animator removeAllBehaviors];
    }
    
    __block UIGravityBehavior *gravityBehaviour = [[UIGravityBehavior alloc] initWithItems:@[self]];
    gravityBehaviour.gravityDirection = CGVectorMake(0.0f, 10.0f);
    [animator addBehavior:gravityBehaviour];
    
    __block UIDynamicItemBehavior *itemBehaviour = [[UIDynamicItemBehavior alloc] initWithItems:@[self]];
